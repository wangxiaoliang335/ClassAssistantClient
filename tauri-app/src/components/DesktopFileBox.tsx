...
import { getCurrentWebviewWindow } from '@tauri-apps/api/webviewWindow';
import { invoke } from '@tauri-apps/api/core';
import { X, Folder, File, FileText, Plus, ArrowRight, Layers, LayoutGrid, RotateCcw, Link2, Package, Code, Terminal } from 'lucide-react';
import { listFiles, releaseBoxToDesktop, createFolder, renameFile, openFile, FileItem } from '../utils/DesktopManager';
import { rename, copyFile, remove, readDir, mkdir, exists, stat, readFile, BaseDirectory } from '@tauri-apps/plugin-fs';
import { join, desktopDir, documentDir } from '@tauri-apps/api/path';
...
const DesktopFileBox = () => {
    ...
    const [desktopBase, setDesktopBase] = useState('');
    const systemIconsRef = useRef<Record<string, string>>({});
    const iconFetchTokenRef = useRef(0);
    const [imageThumbs, setImageThumbs] = useState<Record<string, string>>({});
    const imageThumbsRef = useRef<Record<string, string>>({});
    ...
    useEffect(() => {
        documentDir().then(setDocumentBase).catch(() => setDocumentBase(''));
        desktopDir().then(setDesktopBase).catch(() => setDesktopBase(''));
    }, []);

    const resolveBase = (path: string) => {
        const normalize = (p: string) => p.replace(/[\\/]+$/, '');
        const lower = path.toLowerCase();
        if (documentBase) {
            const base = normalize(documentBase);
            if (lower.startsWith(base.toLowerCase())) {
                const rel = path.slice(base.length).replace(/^[/\\]/, '');
                return { rel, baseDir: BaseDirectory.Document };
            }
        }
        if (desktopBase) {
            const base = normalize(desktopBase);
            if (lower.startsWith(base.toLowerCase())) {
                const rel = path.slice(base.length).replace(/^[/\\]/, '');
                return { rel, baseDir: BaseDirectory.Desktop };
            }
        }
        return null;
    };
    ...
    const fsExists = async (path: string) => {
        const resolved = resolveBase(path);
        if (resolved) {
            return await exists(resolved.rel, { baseDir: resolved.baseDir });
        }
    ...
    const handleDragStart = async (e: React.DragEvent, file: FileItem) => {
        // Try native drag first, fallback to HTML5 drag if not supported
        const isSelected = selectedPaths.includes(file.path);
        const paths = isSelected ? selectedPaths : [file.path];
        if (!isSelected) setSelectedPaths([file.path]);
        ...
        try {
            ...
            // Fallback to HTML5 drag API
            console.log('[FileBox] Native drag not available, using HTML5 drag API');
            // Note: HTML5 drag API may not work for dragging files to desktop
            // Users can use context menu "移动到桌面" or "复制到桌面" instead
            if (e.dataTransfer) {
                e.dataTransfer.effectAllowed = 'copyMove';
                // Set file paths as data - this may not work for dragging to desktop
                e.dataTransfer.setData('application/x-file-path', paths.join('\n'));
            }
        }
    };
    ...
    useEffect(() => {
        // Prefetch Windows system icons so UI matches Explorer as closely as possible.
        // We also fetch for images to use as fallback while thumbnails load.
        const token = ++iconFetchTokenRef.current;
    ...
    useEffect(() => {
        ...
        const setupListeners = async () => {
            ...
            const handleDropPayloadInner = async (event: any, source: string) => {
                ...
                if (paths && paths.length > 0) {
                    try {
                        console.log('[FileBox] Importing files to box via backend...');
                        await invoke('import_files_to_box', { boxId, paths });
                        console.log('[FileBox] Import complete, reloading...');
                        loadFiles();
                    } catch (err) {
                        console.error('[FileBox] Import failed:', err);
    ...
    const handleRelease = async (options?: { silent?: boolean }) => {
        if (!boxId) return;
        if (!options?.silent) {
            if (!confirm('确定要收起此文件盒子吗？所有文件将移动回桌面。')) return;
        }
        await releaseBoxToDesktop(boxId);
        await getCurrentWebviewWindow().close();
    };
    ...
    return (
        <div className="h-screen w-screen overflow-hidden relative font-['Segoe_UI',system-ui] text-gray-800 bg-[#f3f3f3]">
            <div className="absolute inset-0 z-0 bg-white/40 backdrop-blur-[50px]"></div>
            <div
                className={`relative z-10 h-full w-full flex flex-col overflow-hidden select-none transition-all duration-300 rounded-2xl border border-gray-400/30 shadow-[0_0_15px_rgba(0,0,0,0.3)] ${
                    isDragOver ? 'ring-4 ring-inset ring-blue-400/70 bg-blue-50/70' : 'bg-white/70'
                }`}
                onContextMenu={(e) => handleContextMenu(e)}
                onClick={() => { setContextMenu(null); setSelectedPaths([]); }}
                onDragOver={(e) => {
            ...
            <div data-tauri-drag-region className="flex items-center justify-between px-4 py-3 bg-gradient-to-b from-white/50 to-white/10 border-b border-gray-200/50 cursor-move shrink-0">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <button
                        onClick={() => handleRelease({ silent: true })}
                        className="p-1.5 hover:bg-red-100/50 rounded-lg text-gray-400 hover:text-red-600 transition-colors ml-1"
                    >
                        <X size={16} />
                    </button>
